...
//let number$ = Rx.Observable.of(1,2,3);

//number$.subscribe( x => console.log(x) );

//const tick$ = Observable.interval(TICK_MS);

//tick$.subscribe( x=> console.log(x) );
// composition helper

const combine = R.curry((c, o) => x => (<div>{c(x)} {o(x)}</div>))
const combineComponents = (...args) => { // eslint-disable-line
...
const render = getRender(document.body)
